...
### FTP
Simply fire-up the FTP, you can use command-line or GUI to achieve this, nothing big.
<br>
...
If you take some time in exploring the FTP, you will find two folders: "Backup" and "Engineer". To make everything easier, you can download everything first.
<br>
...
You can use the command `get` to download files from an FTP server, but it won't work everytime. If you use the command-line FTP from Linux you should get the same error as me (unless you've configured it before), though I haven't tried the GUI or the Windows version of the FTP client.
...
**ASCII mode** will modify the files transferred (the line-feed) if the communication is 
between two different systems with different line-feed convention (like Linux and Windows).
**Binary mode**, on the other hand will transfer the files without any modification, 
and this is what we want at the moment (and ideally this is what we should use on any FTP transfer).
```
ftp> bin
...
```
> the `--no-passive-ftp` argument disables the use of passive FTP transfer mode (it has something to do about firewall 
and stuff because sometimes the transfer doesn't work using the passive mode) and the `-r` argument tells that we want to
download the file recursively. Note that the default depth of `-r` is 5, remember this if you want to use it later in real life.
<br>
...
Everything's good, let's proceed by checking what kind of files are they.
<br>
...
Look's like we need to go Windows to open these files. But before that, why don't we check the Telnet service while we're still at Linux?
<br>
<br>

### Telnet
You can use the `telnet` command from Linux.
<br>
...
The **Access Control.zip** is password protected, and we simply don’t have the power and time to crack it manually, leaving us with the other file. The **backup.mdb** is a Microsoft Access file, as been reported by the `file` command. We can open it with MS Access and see if there's anything of importance.
<br>
